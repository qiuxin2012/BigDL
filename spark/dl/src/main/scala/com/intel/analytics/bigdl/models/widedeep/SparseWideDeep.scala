...
object WideDeep {
  def apply[T: ClassTag](modelType: String, classNum: Int = 2)
    ...
    // indicator
    deepColumn.add(Sequential().add(Narrow(2, 5007, 33)).add(Reshape(Array(33))
      .setName("indicator")))
    // occupation 1000
...
object SparseWideDeep {
  def apply[T: ClassTag](modelType: String, classNum: Int = 2)
    ...
    deepColumn.add(Sequential().add(Narrow(2, 1, 33)).add(Reshape(Array(33))
      .setName("indicator")))
    // native_country 1000
    deepColumn.add(Sequential().add(Select(2, 34))
      .add(LookupTable(1000, 8, 0.0, 10)
      .setName("embedding_1")))
    // occupation 1000
    deepColumn.add(Sequential().add(Select(2, 35))
      .add(LookupTable(1000, 8, 0.0, 10)
      .setName("embedding_2")))
    // numeric column
