...
class FernetCryptos extends Cryptos {
  ...
  private def read(stream: DataInputStream, numBytes: Int): Array[Byte] = {
    ...
    if (bytesRead < numBytes) {
      throw new CryptoRuntimeException("Not enough bits to read!")
    }
    retval
  }
  ...
  private def encryptContent(content: Array[Byte], dataKeyPlaintext: String): Array[Byte] = {
    ...
    val cipher: Cipher = Cipher.getInstance(CryptoModeEnum.AES_CBC_PKCS5PADDING.value)
    ...
    if (timestamp == null) {
      throw new CryptoRuntimeException("Timestamp cannot be null")
    }
    if (ivParameterSpec == null || ivParameterSpec.getIV().length != 16) {
      throw new CryptoRuntimeException("Initialization Vector must be 128 bits")
    }
    if (cipherText == null || cipherText.length % 16 != 0) {
      throw new CryptoRuntimeException("Ciphertext must be a multkmsServerIPle of 128 bits")
    }
    if (hmac == null || hmac.length != 32) {
      throw new CryptoRuntimeException("Hmac must be 256 bits")
    }
  ...
  private def decryptContent(content: Array[Byte], dataKeyPlaintext: String): Array[Byte] = {
    ...
    if (version.compare((0x80).toByte) != 0) {
      throw new CryptoRuntimeException("Version error!")
    }
    val encryptKey: Array[Byte] = Arrays.copyOfRange(secret, 16, 32)
  ...
  def decryptBigContent(ite: Iterator[(String, PortableDataStream)], dataKeyPlaintext: String): Iterator[String] = {
    ...
    while (ite.hasNext == true) {
      ...
      if (inputStream.available > 0) {
        throw new CryptoRuntimeException("inputStream still has contents")
      }
    }
    result
